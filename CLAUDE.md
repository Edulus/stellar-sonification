...
npm run verify:sky       # browser: ground toggle, live/custom clock, and that both stay
                         ...
                         #   (needs `npm run dev` running + Playwright)
npm run verify:phoenix   # Python/browser-free: PHOENIX extraction helpers + NIST log(gf) path
```

There is no lint step and no general test runner. The JavaScript `verify:*` scripts in `scripts/` are plain Node `.mjs` files; `verify:phoenix` runs `data-pipeline/verify_phoenix.py`. Playwright is not a declared dependency — it must be installed in the environment for the browser verifiers. `verify:extract`, `verify:seed`, and `verify:phoenix` are browser-free; `verify:engine`, `verify:slice`, `verify:tooltip` and `verify:sky` need `npm run dev` running first.
...
- **The `src/data/*.js` data modules are generated adapters.** `bright-stars.js` / `spectral-templates.js` import the committed JSON in [data-pipeline/output/](data-pipeline/output/) (the source of truth) and reshape it. Edit the **pipeline** (`data-pipeline/build-*.py`, or `common.py::FEATURES` for template line strengths) and rerun, not the JS. The default template build uses the compact model; `build-templates.py --phoenix` now extracts from local PHOENIX-ACES HiRes FITS where the grid covers the key. See [data-pipeline/PHOENIX.md](data-pipeline/PHOENIX.md).
- **PHOENIX data files are local inputs.** The multi-GB grid is git-ignored under `data-pipeline/downloads/phoenix/`. The extraction code, continuum sweep, diagnostics, NIST `log(gf)` blend path and fallback policy are implemented, but the committed `spectral-templates.json` remains the existing model output until a real local grid run is performed and reviewed.

## Repository notes

The code is authoritative when older narrative/planning material disagrees. Current implementation notes that are easy to misread from historical files:

- Spectral data reaches the app as JS modules (`bright-stars.js`, `spectral-templates.js`) that are thin adapters over committed [data-pipeline/output/](data-pipeline/output/) JSON. `catalogs.js` is still not built.
...
- [harmonize.js](harmonize.js) (repo root, co-located with the prototype) is the standalone browser-free harmonizer used by **both** the prototype and the `src/` app; [src/audio/harmonize.js](src/audio/harmonize.js) re-exports it.

[docs/ROADMAP.md](docs/ROADMAP.md) tracks phase status. Phase 4 remains the active data-expansion phase: the PHOENIX extraction path is implemented, while a real-grid calibration/run and broader catalog/survey expansion remain open. The engine is AGPL-3.0 — relevant if distributing modified engine builds.
