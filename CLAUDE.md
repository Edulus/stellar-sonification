...
- UI is `SkyCanvas.jsx` + `SpectrumPanel.jsx` + `SynthPanel.jsx` + `LocationPicker.jsx` + `RingOverlay.jsx` + `ObjectTooltip.jsx`; the docs' `StarInfoPanel` / `ParamMappingPanel` are not built — `ObjectTooltip` covers part of what `StarInfoPanel` was meant to do, and `SynthPanel` covers `ParamMappingPanel`.
